...
Select a single **rectangle** layer to bind it to a Base64-encoded image stored in the bound XML, instead of managing XPath annotations. Rectangles work like text layers for this purpose: enter or pick an XPath pointing at the element/attribute holding the image data, check the live `<img>` preview, and click **Apply picture** to upload the decoded image to Penpot and set it as the rectangle's fill.

The image format (JPEG, PNG, GIF or WEBP) is auto-detected from the decoded bytes; there is no manual toggle. Unsupported or unrecognized data is reported as an error and the fill is left unchanged. A rectangle cannot hold XPath annotations — it is always either unbound or bound to a picture.

Picture layers fully participate in repeater containers: a rectangle among a container's direct children (or their descendants) can be bound to a relative path such as `./photo`, and **Apply**/**Refresh all bindings** upload and set the matching image for every generated instance, same as text fields. Repeated identical image values (for example the same photo across several repeater instances) are uploaded only once per plugin session and reused. As with text layers, only the **original** picture layer inside a repeater template can be edited directly; generated clones show the same read-only note and reject direct edits.

## Repeater containers

Select a **board** to reveal the **Repeater** section, in addition to its XPath entries. This board acts as the **repeater container**: bind the text layers among its direct child layers (and their own descendants) to relative XPaths (for example `./name` instead of `/data/items/item/name`) using their own single binding as usual; the section lists these bound fields for reference, marking picture fields with `[picture]`. Enter a **Repeater container: XPath (node-set)** that selects the repeating elements (for example `/data/items/item`), or click **Select nodes** to pick one of the repeating elements from the XML tree — unlike the single-value picker, this writes the path without a position or `@id` predicate so it matches every sibling of that kind. Click **Apply** to generate the instances; nothing changes until you do.


For each matched element, the plugin clones every direct child layer of the container as a group and resolves every bound text or picture field's path relative to that specific element (native XPath: paths starting with `/` still resolve from the document root, so absolute bindings inside a repeater container are unaffected). The first match updates the existing child layers in place; the rest are added as cloned groups stacked vertically below the template, preserving the children's relative arrangement, with a fixed gap between groups. Re-clicking **Apply** removes the previously generated clones first, so it never duplicates; **Remove repeater** deletes all generated clones and clears the configuration, leaving the container's original child layers and their text/picture values untouched.


When you select a text or picture (rectangle) layer that is nested (at any depth) inside a repeater container, its value preview, **Apply binding**/**Apply picture** and **Choose XML node** automatically use the repeater's matched element as context (shown as a note above the value preview) as soon as a container XPath exists — even a still-unsaved draft you just typed — so a relative path like `./name` resolves and can be applied normally instead of showing "XPath has no matches" against the whole document. The picker also starts its tree at that matched element and generates paths relative to it. This only affects the preview/picker for that one representative match; the real per-instance values are still computed separately for every match whenever you click **Apply** on the container.

Only the **original** text or picture layers inside a container's template can have their XPath edited. A layer that belongs to a generated clone (any non-first match) shows a note instead — its XPath field, picker and Apply binding/Apply picture are disabled, since its value is fully controlled by the repeater and would just be overwritten on the next regenerate; edit the corresponding layer inside the template instead.

**Refresh all bindings** also regenerates every repeater container on the current page: it discards each container's previously generated clones and recreates them from the latest XML, in addition to updating plain text and picture bindings.
