...
When you select a text layer that is nested inside a board with a saved repeater configuration, its value preview, **Apply binding** and **Choose XML node** automatically use the repeater's first matched element as context (shown as a note above the value preview), so a relative path like `./name` resolves and can be applied normally instead of showing "XPath has no matches" against the whole document. The picker also starts its tree at that matched element and generates paths relative to it. This only affects the preview/picker for that one representative match; the real per-instance values are still computed separately for every match when you click **Generate instances** on the board.
